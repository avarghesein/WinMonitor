...
extern bool SetKeyBoardHook(void);
extern bool UnInstallKeyBoardHook(void);
...
{	
	...
	try
	{
		CTString ExeName,CmdLine;		
		if(!CTString((TCHAR*)m_tchrCommandArguments).SplitByToken(_T("\n"),ExeName,CmdLine)) return DefaultMessage(); 
...
bool CCommandMonitorServer::CreateFolderFunc(void)
{
	if(!m_tchrCommandArguments) return DefaultMessage();
	bool blnOk=true;	
	if(!CTString((TCHAR*)m_tchrCommandArguments).CreatePath()) blnOk=false;
...
bool CCommandMonitorServer::OpenOrCloseCDRom(bool IsOpen)
{
	...
	CGeneralMemoryUtility::DeleteAll((void**)&m_tchrCommandResult);  
	m_ulngResultLength = 0;
	if((IsOpen ? FunPlugin.OpenCDRom() : FunPlugin.CloseCDRom())) return true;
	return DefaultMessage();   
}
