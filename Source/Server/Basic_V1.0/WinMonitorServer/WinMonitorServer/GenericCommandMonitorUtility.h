...
{
	CTString Path,RelFileName;

	Path = CTString(FileName).GetDirectoryPart(); 
	RelFileName = CTString(FileName).GetFileNamePart(); 
	if(Path == (UINT64)0 || RelFileName == (UINT64)0) return false; 
...
bool CCommandMonitorUtility::SaveFileToDisk(TCHAR *FileName,TCHAR *DstRootDirPath,BYTE *DataStream,UINT32 StreamLength)
{
	...
	try 
	{
		HFILE File=_open(Path,_O_CREAT | _O_TRUNC | _O_WRONLY | _O_BINARY ,_S_IREAD | _S_IWRITE );
