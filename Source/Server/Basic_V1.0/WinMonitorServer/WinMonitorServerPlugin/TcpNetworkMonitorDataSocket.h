...
bool CTcpNwMonitorConnection::Disconnect(void)
{
	 ...
	 { 
			if(m_muxConnection) 
				try { CloseHandle(m_muxConnection); } catch(...) {}
			m_muxConnection=0;
			if(!m_boolErrorFlag && m_socketDataSocket != -1)
			{
	 ...
	 {	
		 m_socketDataSocket = -1;
		 return SetError(CONNECTION_CLOSE_ERROR); 
	 }	 
...
CTcpNwMonitorConnection::CTcpNwMonitorConnection()
{
	 m_muxConnection=0; m_socketDataSocket = -1;
	 this->ReFresh();
}
...
void  CTcpNwMonitorConnection::ReFresh(void) 
{
	 ...
	 {
			Disconnect(); 
			if(m_muxConnection==0) m_muxConnection=CreateMutex(0,0,0); 	 
