...
class CLzhCompress:public CGeneralMemoryUtility  
{
private:
...
int CLzhCompress::fnc_write_memory(BYTE *pBuffer, int nLen)
{
	...
	if((*ppMemOut)==NULL) {
		nMemRealAlloc=nLen*3/2+1024;
		if(!AllocateMem((void**)ppMemOut,nMemRealAlloc)) return -1;
	}

	if((nCount+nMemOutOff)>nMemRealAlloc) {
		int nAlloc=(nMemOutOff+nCount)*3/2+1024;
		void *newmem=0;
		if(!AllocateMem((void**)&newmem,nAlloc)) return -1;

		memcpy(newmem,*ppMemOut,nMemOutOff);
		DeleteAll((void**)ppMemOut); 
		*ppMemOut=(BYTE *)newmem;
		nMemRealAlloc=nAlloc;
	}
	
	memcpy(*ppMemOut+nMemOutOff,pBuffer,nCount);
	nMemOutOff+=nCount;
	*pnMemOutSize=nMemOutOff;
...
int CLzhCompress::lzh_freeze_file(const char *szInFile, const char *szOutFile)
{
...
int CLzhCompress::lzh_freeze_memory(void *pInBuffer, int nInBufLen, void **ppOutBuf, int *pnOutBufLen)
{
	pMemIn=(BYTE *)pInBuffer;
	nMemInSize=nInBufLen;
	ppMemOut=(BYTE **)ppOutBuf;
	pnMemOutSize=pnOutBufLen;
	nMemInOff=0;
	nMemOutOff=0;
...
int CLzhCompress::lzh_melt_file(const char *szInFile, const char *szOutFile)
{
	hFileIn=CreateFile(szInFile,GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,0,NULL);
	if(hFileIn==INVALID_HANDLE_VALUE) return -1;
	hFileOut=CreateFile(szOutFile,GENERIC_WRITE,0,NULL,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,NULL);
...
int CLzhCompress::lzh_melt_memory(void *pInBuffer, int nInBufLen, void **ppOutBuf, int *pnOutBufLen)
{
	pMemIn=(BYTE *)pInBuffer;
	nMemInSize=nInBufLen;
	ppMemOut=(BYTE **)ppOutBuf;
...
int CLzhCompress::lzh_free_memory(void **ppOutBuf)
{
	DeleteAll((void**)ppOutBuf);
	*ppOutBuf=NULL;
	return 0;
}
