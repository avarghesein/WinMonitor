...
class CScreenMonitorMessenger:public INetworkByteStream,public CGeneralMemoryUtility   
{
	protected:
		ScreenMonitorNetworkeader m_NwHeader;
		BYTE *m_bytptrArgs;
	public:
		CScreenMonitorMessenger(void);
		~CScreenMonitorMessenger();
		UINT32 WriteToNetworkByteStream(BYTE* &OutByteStream);
		UINT32 ReadFromNetworkByteStream(BYTE *InByteStream);
		bool SetMessageType(BYTE MsgType,BYTE *MsgArgs=0,long ArgLen=0);
};

CScreenMonitorMessenger::CScreenMonitorMessenger(void)
{
	m_bytptrArgs=0;
}

CScreenMonitorMessenger::~CScreenMonitorMessenger()
{
	DeleteAll((void**)&m_bytptrArgs); 
}

bool CScreenMonitorMessenger::SetMessageType(BYTE MsgType,BYTE *MsgArgs,long ArgLen)
{
	m_NwHeader.TypeAndCompression=SCREEN_MONITOR_SETTYPE(SCREEN_MONITOR_TYPE_UNDEF,m_NwHeader.TypeAndCompression); 
	m_NwHeader.TypeAndCompression=SCREEN_MONITOR_SETCOMPRESSION(SCREEN_MONITOR_COMPRESS_NIL,m_NwHeader.TypeAndCompression); 
	m_NwHeader.Length=0; 
	DeleteAll((void**)&m_bytptrArgs);
	if(MsgArgs && ArgLen>0)
	{
		try
		{	
			if(!AllocateMem((void**)&m_bytptrArgs,sizeof(BYTE)*ArgLen) ||
			   !CopyBytes(m_bytptrArgs,MsgArgs,ArgLen)) return false;
			m_NwHeader.Length=ArgLen; 
		}
		catch(...) { return false; }
	}
	m_NwHeader.TypeAndCompression=SCREEN_MONITOR_SETTYPE(MsgType,m_NwHeader.TypeAndCompression); 
	return true;
}

UINT32 CScreenMonitorMessenger::WriteToNetworkByteStream(BYTE* &OutByteStream)
{
	DeleteAll((void**)&OutByteStream);
	try
	{	if(!AllocateMem((void**)&OutByteStream,sizeof(BYTE)*(sizeof(WinMonitorNetworkServiceHeader)+sizeof(ScreenMonitorNetworkeader)+m_NwHeader.Length)))
			return false;
		BYTE *OutStream=(BYTE*)(OutByteStream+sizeof(WinMonitorNetworkServiceHeader));
		*((ScreenMonitorNetworkeader*)OutStream)=m_NwHeader;
		if(m_NwHeader.Length>0) CopyBytes((BYTE*)(OutStream+sizeof(ScreenMonitorNetworkeader)),m_bytptrArgs,m_NwHeader.Length);  
		return sizeof(ScreenMonitorNetworkeader)+m_NwHeader.Length;
	}
	catch(...) { return 0; }
}


UINT32 CScreenMonitorMessenger::ReadFromNetworkByteStream(BYTE *InByteStream){ return 0; }
